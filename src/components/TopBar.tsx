...
export const TopBar = () => {
  ...
  return (
    <header className="sticky top-0 z-20 h-16 bg-background/70 backdrop-blur-xl border-b border-border/50 flex items-center px-6 gap-4 transition-all duration-300">
      {showSearchBar ? (
      <div ref={wrapRef} className="relative flex-1 max-w-2xl">
        ...
        <Input
          ...
          className="pl-11 pr-20 h-11 bg-secondary/60 border border-border rounded-full focus-visible:ring-2 focus-visible:ring-primary/40 focus-visible:border-primary/40 text-sm transition-all"
        />
        {q.trim() && (
          <Button
            type="button"
            ...
            className="absolute right-10 top-1/2 -translate-y-1/2 h-7 w-7 rounded-full text-primary hover:text-primary hover:bg-primary/10"
            aria-label="Search"
            title="Search"
          >
        ...
        {showSuggest && suggestions.length > 0 && (
          <div className="absolute left-0 right-0 mt-2 bg-popover border border-border rounded-xl shadow-2xl overflow-hidden z-30 animate-fade-in">
